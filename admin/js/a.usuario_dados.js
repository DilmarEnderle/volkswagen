...
function salvarMeusDados()
{
	...
	if (ajax)
	{
		...
		ajax = false;
		var dataString = $("#usuario-form").serialize()+"&f-notificacoes="+($("#i-notif-sim").hasClass("check-dot1") << 0);
		$.ajax({
			type: "POST",
			url: "a.usuario_dados_salvar.php",
			data: dataString,
			dataType: "json",
			success: function(data)
			{
				...
				if (data[0] == 9)
					...
					ultimateDialog({title:'Sucesso!',color:'green',content:'Alterações salvas com sucesso.',buttons:{'Ok':{is_default:1}}});
				else
