...
if (isInside())
{
	...
	$pMotivo = preg_replace("/\s+/", " ", $db->escapeString(strip_tags(trim(utf8_decode($_POST["motivo"])))));
	...
	$db->query("SELECT id FROM gelic_comprasrp_historico WHERE id_comprasrp = $pId_cd AND tipo IN (4,5) LIMIT 1");
	if (!$db->nextRecord())
	{
		$db->query("
SELECT
...
<span style="font-weight: bold;">Data do SRP:</span> '.mysqlToBr($dData_srp).'<br>';
			...
			//nao autorizar
			$db->query("INSERT INTO gelic_comprasrp_historico VALUES (NULL, $pId_cd, 0, $sInside_id, 6, 0, '$now', '$pMotivo', '', '')");


			//-----------------------------------------
			//--- ENVIO DE NOTIFICACOES EMAIL / SMS ---
			//-----------------------------------------
			$tEmail_assunto = 'GELIC - O envio da APL não foi autorizado (Compra Direta/SRP)';
			$tEmail_mensagem = 'O envio da APL para a seguinte solicitação de Compra Direta/SRP não foi autorizado.<br><br>
<span style="font-weight:bold;">Motivo: </span>'.$pMotivo.'<br>
...
'.rodapeEmail();
			$tTexto_sms = 'GELIC - O envio da APL não foi autorizado (Compra Direta/SRP)';
