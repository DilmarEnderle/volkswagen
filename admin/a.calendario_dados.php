...
if (isInside())
{
	...
	if ($pYear >= $pYear_min && $pYear <= $pYear_max && checkdate($pMonth,$pDay,$pYear))
	{
		...
		if ($pType == 1) 
		{
			...
			//***             MES             ***
			//***********************************
			$tOutput = '<div class="t14 bold cal_title">'.$aMonth[$pMonth].', '.$pYear.'<br><a class="t12 normal">'.$aOrder[$pOrder].'</a> <a id="printer" href="javascript:doPrint();" title="Imprimir" style="position: absolute; right: 0; top: 0;"><img src="img/print.png" style="border: 0;"></a></div><div class="cal_cap" style="border-left: 1px solid #000000;">Domingo</div><div class="cal_cap">Segunda</div><div class="cal_cap">Terça</div><div class="cal_cap">Quarta</div><div class="cal_cap">Quinta</div><div class="cal_cap">Sexta</div><div class="cal_cap">Sábado</div>';
...
SELECT
	id,
	valor,
	DAY(datahora_abertura) AS dia
FROM
	gelic_licitacoes
WHERE
	deletado = 0 AND
	datahora_abertura >= '$frDate' AND
	datahora_abertura <= '$toDate'");
			}
			else if ($pOrder == 1)
			{
				// ---------- COM APL ENVIADA ----------
...
SELECT
	lic.id,
	lic.valor,
	DAY(lic.datahora_abertura) AS dia
FROM
...
GROUP BY
	lic.id");
			...
			for ($i=1; $i<=$days_in_month; $i++)
			{
				...
				if (array_key_exists($i,$data))
				{
					...
					for ($k=0; $k<count($data[$i]); $k++)
						$rows .= '<a class="cal-btn" href="a.licitacao_abrir.php?id='.$data[$i][$k][1].'">'.$data[$i][$k][1].' (R$ '.$data[$i][$k][2].')</a>';
		...
		else if ($pType == 2)
		{
			...
			$tOutput = '<div class="t14 bold cal_title">Semana de '.mysqlToBr(substr($frDate,0,10)).' à '.mysqlToBr(substr($toDate,0,10)).'<br><a class="t12 normal">'.$aOrder[$pOrder].'</a> <a id="printer" href="javascript:doPrint();" title="Imprimir" style="position: absolute; right: 0; top: 0;"><img src="img/print.png" style="border: 0;"></a></div><div class="cal_cap" style="border-left: 1px solid #000000;">Domingo</div><div class="cal_cap">Segunda</div><div class="cal_cap">Terça</div><div class="cal_cap">Quarta</div><div class="cal_cap">Quinta</div><div class="cal_cap">Sexta</div><div class="cal_cap">Sábado</div>';
			$data = array(); //holds the info from the database for faster processing
...
SELECT
	id,
	valor,
	DATE(datahora_abertura) AS data
FROM
	gelic_licitacoes
WHERE
	deletado = 0 AND
			...
			for ($i=1; $i<8; $i++)
			{
				...
				if (array_key_exists($this_date_db, $data))
				{
					...
					for ($k=0; $k<count($data[$this_date_db]); $k++)
						$rows .= '<a class="cal-btn" href="a.licitacao_abrir.php?id='.$data[$this_date_db][$k][1].'">'.$data[$this_date_db][$k][1].' (R$ '.$data[$this_date_db][$k][2].')</a>';
	...
	lic.id");
			...
			if (array_key_exists(1,$data))
			{
				...
				for ($k=0; $k<count($data[1]); $k++)
					$rows .= '<a class="cal-btn" href="a.licitacao_abrir.php?id='.$data[1][$k][1].'">'.$data[1][$k][1].' (R$ '.$data[1][$k][2].')</a>';
			...
			$tOutput = '<div class="t14 bold cal_title">Dia '.str_pad($pDay,2,"0",STR_PAD_LEFT).'/'.str_pad($pMonth,2,"0",STR_PAD_LEFT).'/'.$pYear.'<br><a class="t12 normal">'.$aOrder[$pOrder].'</a> <a id="printer" href="javascript:doPrint();" title="Imprimir" style="position: absolute; right: 0; top: 0;"><img src="img/print.png" style="border: 0;"></a></div><div class="cal_cap" style="border-left: 1px solid #000000; width: 1084px;">'.$aWeek[$day_of_week].'</div>';
			$tOutput .= '<div class="cal_data_mo" style="border-left: 1px solid #000000; width: 1084px;">'.$box_data.'</div>';
		}
	}
	else
	{
		$tOutput = 'Data inválida!';
	}
