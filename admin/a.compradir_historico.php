		...
		if ($db->f("tipo") == 1 && !$autorizado) //(DN) 1: Solicitação enviada. Aguardando autorização...
		{
			...
			$oOutput .= '<div class="m-row">
				<div class="m-row-receive">
					...
					<div class="m-txt">
						<p><span class="italic gray-88">Aguardando autorização...</span></p>
					</div>
				</div>
			</div>';
		}
		else if ($db->f("tipo") == 2) //(GL) 2: Mensagem enviada pelo ADMIN
		{
			...
			if (strlen($db->f("arquivo")) > 0)
				$oOutput .= '<p>
					...
					<span class="t13 red fl ml-8 lh-20" style="border: 1px solid #bebebe; padding: 0 10px; box-sizing: border-box; border-right: 0;background-color:#ffffff;">'.utf8_encode($db->f("nome_arquivo")).'</span>
					<a class="bt-style-2 fl" href="'.linkFileBucket("vw/cdchat/".$db->f("arquivo")).'" target="_blank" style="font-size: 12px; height: 22px; line-height:20px;">Abrir Anexo</a>
				</p>';
		...
		else if ($db->f("tipo") == 5) //(GL) 5: O envio da APL foi autorizado.
		{
			$oOutput .= '<div class="m-row">
				<div class="m-row-send">
					...
					<div class="m-txt">
						<p><span class="green bold">O envio da APL foi autorizado.</span></p>
					</div>
				</div>
			</div>';
		...
		else if ($db->f("tipo") == 6) //(GL) 6: O envio da APL não foi autorizado.
		{
			$oOutput .= '<div class="m-row">
				<div class="m-row-send">
					...
					<div class="m-txt">
						<p><span class="red bold">O envio da APL não foi autorizado.</span></p>
		...
		else if ($db->f("tipo") == 41) //(DN) 41: APL Enviada
		{
			...
			$oOutput .= '<div class="m-row">
				<div class="m-row-receive">
					...
					<div class="m-txt">
						<p><span class="bold orange">APL enviada</span> por <span class="bold italic">'.$por.'</span> de <span class="red">'.$db->f("ip").'</span></p>
					</div>
				</div>
			</div>';
		}
		else if ($db->f("tipo") == 42) //(BO) 42: APL Aprovada
		{
			...
			$oOutput .= '<div class="m-row">
				<div class="m-row-receive">
					...
					<div class="m-txt">
						<p><span class="bold green">APL aprovada</span> por <span class="bold italic">'.$por.'</span> de <span class="red">'.$db->f("ip").'</span></p>
					</div>
				</div>
			</div>';
		}
		else if ($db->f("tipo") == 43) //(BO) 43: APL Reprovada
		{
			...
			$oOutput .= '<div class="m-row">
				<div class="m-row-receive">
					...
					<div class="m-txt">
						<p><span class="bold red">APL reprovada</span> por <span class="bold italic">'.$por.'</span> de <span class="red">'.$db->f("ip").'</span></p>
					</div>
				</div>
			</div>';
		}
		else if ($db->f("tipo") == 44) //(BO) 44: APL Aprovação Revertida
		{
			...
			$oOutput .= '<div class="m-row">
				<div class="m-row-receive">
					...
					<div class="m-txt">
						<p><span class="bold blue">Aprovação da APL revertida</span> por <span class="bold italic">'.$por.'</span> de <span class="red">'.$db->f("ip").'</span>'.$obs.'</p>
					</div>
				</div>
			</div>';
		}
		else if ($db->f("tipo") == 45) //(BO) 45: APL Reprovação Revertida
		{
			...
			$oOutput .= '<div class="m-row">
				<div class="m-row-receive">
					...
					<div class="m-txt">
						<p><span class="bold blue">Reprovação da APL revertida</span> por <span class="bold italic">'.$por.'</span> de <span class="red">'.$db->f("ip").'</span>'.$obs.'</p>
					</div>
				</div>
			</div>';
		}
