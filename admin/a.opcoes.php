...
if (isInside())
{
	...
	if (!in_array("cnf_visualizar", $xAccess))
	{
		...
		$tPage->replace("{{MSG}}", '<span class="bold">CONFIGURAÇÕES</span><br><br><span class="gray-88">Você não tem permissão neste módulo.</span>');
	...
	$tPage = new Template("a.opcoes.html");
	$tPage->replace("{{SEC_TOP}}", utf8_encode(getSec_top()));
