...
if (isInside())
{
	...
	if ($pFormato == "xlsx")
	{
		...
		$phpexcel->getProperties()->setCreator("GELIC")
			...
			->setTitle("Licitacoes - Dados Gerais")
			->setSubject("Licitacoes")
			->setDescription("Licitacoes Dados Gerais GELIC")
	...
	lic.id DESC, lot.lote, itm.item");
		while ($db->nextRecord())
		{
			...
			if (in_array($db->f("id_status"), array(8,19))) // APL Aprovada, APL Reprovada
			{
				...
				if ($db->f("fase") == 1)
				{
					if ($db->f("enviadas",1) > 0)
						$dStatus .= ' APL em Análise - GELIC ('.$db->f("enviadas",1).')';
				}
				else
				{
					if ($db->f("enviadas",1) > 0)
						$dStatus .= ' APL Aguardando Aprovação ('.$db->f("enviadas",1).')';
				}

				if ($db->f("aprovadas",1) > 0)
					$dStatus .= ' APL Aprovada ('.$db->f("aprovadas",1).')';
	
				if ($db->f("reprovadas",1) > 0)
					$dStatus .= ' APL Reprovada ('.$db->f("reprovadas",1).')';
			}
			else
			{
				$db->query("SELECT descricao FROM gelic_status WHERE id = ".$db->f("id_status"),1);
				$db->nextRecord(1);
				$dStatus = utf8_encode($db->f("descricao",1));
			}

			$dStatus = trim($dStatus);
			...
			if ($db->f("lote") <> '')
			{
				...
				if ($dQuantidade == 0) $dQuantidade = 1;
			...
			$phpexcel->setActiveSheetIndex(0)
				...
				->setCellValue("Q$row", $dStatus);
		}


		$phpexcel->getActiveSheet()->setTitle('Licitações - Dados Gerais');
		...
		$phpexcel->getActiveSheet()->getStyle("P3:P$row")->getNumberFormat()->setFormatCode('[$R$-416] #.##0,00;[RED]-[$R$-416] #.##0,00');
		...
		$phpexcel->getActiveSheet()->getStyle("Q2:Q$row")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		$phpexcel->getActiveSheet()->getStyle("A1:Q$row")->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);

		if (file_exists(UPLOAD_DIR."~licitacoes_dados_".$sInside_id.".xlsx"))
			unlink(UPLOAD_DIR."~licitacoes_dados_".$sInside_id.".xlsx");

		$obwriter = PHPExcel_IOFactory::createWriter($phpexcel, 'Excel2007');
		$obwriter->save(UPLOAD_DIR."~licitacoes_dados_".$sInside_id.".xlsx");
