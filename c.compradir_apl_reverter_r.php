...
if (isInside())
{
	...
	//verificar se esta APL foi reprovada
	...
	else
	{
		...
		//alterar APL historico tipo 4 para tipo 8
		$db->query("UPDATE gelic_comprasrp_apl_historico SET tipo = 8 WHERE id = $dId_apl_historico");

		//adicionar APL historico tipo 6
		$ip = $_SERVER['REMOTE_ADDR'];
		$db->query("INSERT INTO gelic_comprasrp_apl_historico VALUES (NULL, $pId_apl, $sInside_id, 6, '$ip', 0, 0, '$now', '$pObservacoes')");
		$dId_apl_historico = $db->li();

		//adicionar historico tipo 45 (APL Reprovação Revertida)
		$db->query("INSERT INTO gelic_comprasrp_historico VALUES (NULL, $dId_cd, $pId_apl, $sInside_id, 45, $dId_apl_historico, '$now', '', '', '')");
...
<span style="font-weight: bold;">Data do SRP:</span> '.mysqlToBr($db->f("data_srp")).'<br>';
		...
		$tEmail_assunto = 'GELIC - Reprovação Revertida (COMPRA DIRETA/SRP)';
		$tEmail_mensagem = 'APL Reprovação Revertida referente a seguinte solicitação de Compra Direta/SRP.<br><br>
...
'.rodapeEmail();
		$tTexto_sms = 'GELIC - APL Reprovação Revertida (COMPRA DIRETA/SRP) DN: '.utf8_encode($db->f("nome_dn"));
		...
		while ($db->nextRecord())
		{
			if (in_array("P", str_split($db->f("nt_email"))))
				queueMessage(16, 0, basename(__FILE__).' ('.__LINE__.')', $sInside_id, M_EMAIL, BOF_ADM, $db->f("id"), $db->f("email"), $tEmail_assunto, $tEmail_mensagem, '', '');

			if (in_array("P", str_split($db->f("nt_celular"))) && strlen($db->f("celular")) > 0)
				queueMessage(16, 0, basename(__FILE__).' ('.__LINE__.')', $sInside_id, M_SMS, BOF_ADM, $db->f("id"), $db->f("celular"), '', $tTexto_sms, '', '');
		}
...
WHERE
	...
	cli.deletado = 0");
		while ($db->nextRecord())
		{
			...
			if (in_array("R", str_split($dNt_email["ntf"])))
				queueMessage(44, 0, basename(__FILE__).' ('.__LINE__.')', $sInside_id, M_EMAIL, BOF_DLR, $db->f("id"), $db->f("email"), $tEmail_assunto, $tEmail_mensagem, '', '');

			if (in_array("R", str_split($dNt_sms["ntf"])) && strlen($db->f("celular")) > 0)
				queueMessage(44, 0, basename(__FILE__).' ('.__LINE__.')', $sInside_id, M_SMS, BOF_DLR, $db->f("id"), $db->f("celular"), '', $tTexto_sms, '', '');
		}
		//-----------------------------------------
		//-----------------------------------------
		//-----------------------------------------
