...
function salvarAPL_item(id_item, now)
{
	...
	$("#i-garantia-prazo-item").val($.trim($("#i-garantia-prazo-item").val()));
	...
	$("#i-forma-pagamento-item").val($.trim($("#i-forma-pagamento-item").val()));
	...
	if ($("#i-cnpj-faturamento-item").val().length < 18)
	{
	...
	if ($("#i-participante-cpf-item").val().length < 14)
	{
	...
	if ($("#i-garantia-prazo-item").val().length == 0)
	{
		ultimateDialog({title:'Erro.',color:'red',content:'Digite o prazo garantia.',buttons:{'ok':{is_default:1,onclick:"scrollToElement('#i-garantia-prazo-item',false);ultimateClose();"}}});
		return false;
	}
	...
	if ($("#i-forma-pagamento-item").val().length == 0)
	{
		ultimateDialog({title:'Erro.',color:'red',content:'Informe a forma de pagamento.',buttons:{'ok':{is_default:1,onclick:"scrollToElement('#i-forma-pagamento-item',true);ultimateClose();"}}});
		return false;
	}
	...
	if ($('input[name="f-garantia-proposta-item"]').val() == 0)
	{
		ultimateDialog({title:'Erro.',color:'red',content:'Selecione a garantia de proposta.',buttons:{'ok':{is_default:1,onclick:"scrollToElement('#hook-garantia-proposta',false);ultimateClose();"}}});
		return false;
	}

	if ($('input[name="f-garantia-contrato-item"]').val() == 0)
	{
		ultimateDialog({title:'Erro.',color:'red',content:'Selecione a garantia de contrato.',buttons:{'ok':{is_default:1,onclick:"scrollToElement('#hook-garantia-proposta',false);ultimateClose();"}}});
		return false;
	}
	...
	if ($("#cl-amarok").children(".apl-upl-loading").is(":visible") || $("#cl-spacefox").children(".apl-upl-loading").is(":visible"))
	{
