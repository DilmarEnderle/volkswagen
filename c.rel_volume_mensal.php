	...
	dn
		...
		if ($pFormato == "xlsx")
		{
			...
			$phpexcel->getProperties()->setCreator("GELIC")
				...
				->setTitle("Volume de Acessos por Mês")
				->setSubject("Acessos")
			...
			$phpexcel->getActiveSheet()->setTitle('Volume de Acessos por Mês');
			...
			if (file_exists(UPLOAD_DIR."~volume_mensal_".$sInside_id.".xlsx"))
				unlink(UPLOAD_DIR."~volume_mensal_".$sInside_id.".xlsx");

			$obwriter = PHPExcel_IOFactory::createWriter($phpexcel, 'Excel2007');
			$obwriter->save(UPLOAD_DIR."~volume_mensal_".$sInside_id.".xlsx");
...
<head>
	<meta charset="utf-8">
	<title>Volume de Acessos por Mês</title>
	<style>
...
<div style="width: 960px; overflow: hidden;">
	...
	'.$output.'
			...
			$oFile = fopen(UPLOAD_DIR."~volume_mensal_".$sInside_id.".html", "w");
			fwrite($oFile, $tHtml);
			fclose($oFile);

			exec(PATH_HTMTOPDF." --image-quality 100 ".UPLOAD_DIR."~volume_mensal_".$sInside_id.".html ".UPLOAD_DIR."~volume_mensal_".$sInside_id.".pdf");
			@unlink(UPLOAD_DIR."~volume_mensal_".$sInside_id.".html");
