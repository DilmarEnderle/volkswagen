...
if (isInside())
{
	...
	if ($sInside_tipo == 1) //BO
	{
		...
		$db = new Mysql();
		$db->query("SELECT ave, quantidade, model_code, cor, opcionais_pr, preco_publico, prazo_de_entrega, transformacao FROM gelic_comprasrp_apl WHERE id_comprasrp = $pId_cd ORDER BY id DESC LIMIT 1");
		if ($db->nextRecord())
		{
			$dAve = utf8_encode($db->f("ave"));
			$dQuantidade = $db->f("quantidade");
			$dModel = utf8_encode($db->f("model_code"));
			...
			$dPrazo_entrega = $db->f("prazo_de_entrega");
		...
		$db->query("SELECT texto FROM gelic_texto WHERE id = 12");
		$db->nextRecord();
		...
		<div class="ultimate-row" style="background-color:#dedede;">
			<div style="position: relative; float:left;width:462px;overflow:hidden;padding-top:10px;">
				...
				<input id="i-apr-quantidade" class="iText fl" type="text" name="f-apr-quantidade" maxlength="4" value="'.$dQuantidade.'"style="width:100px;height:30px;line-height:28px;font-size:13px;color:#000000;margin-top:3px;">

				<span class="fl lh-30 clear" style="width:100px;padding-left:10px;margin-top:3px;">Model Code <span class="t-red">*</span></span>
				<input id="i-apr-model" class="iText fl" type="text" name="f-apr-model" maxlength="6" value="'.$dModel.'" style="width:100px;height:30px;line-height:28px;font-size:13px;color:#000000;margin-top:3px;">
			...
			<div style="float:left;width:330px;overflow:hidden;padding-top:10px;">
				...
				<input id="i-apr-preco-publico" class="iText fl" type="text" name="f-apr-preco-publico" value="'.$dPreco_publico.'"style="width:180px;height:30px;line-height:28px;font-size:13px;color:#000000;">
