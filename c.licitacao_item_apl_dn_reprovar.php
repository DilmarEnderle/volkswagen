...
if (isInside())
{
	...
	$db->query("SELECT fase, aprovar_apl FROM gelic_licitacoes WHERE id = $pId_licitacao");
	$db->nextRecord();
	$dFase = $db->f("fase");
...
WHERE
	...
	if ($db->nextRecord())
	{
		...
		//REPROVAR
		$db->query("INSERT INTO gelic_licitacoes_apl_historico VALUES (NULL, $dId_apl, $sInside_id, 4, '$ip', $pId_motivo, $pId_submotivo, '$now', '$pObservacoes')");
		$dId_apl_historico = $db->li();
		$db->query("INSERT INTO gelic_historico VALUES (NULL, $pId_licitacao, 0, $dId_apl, $sInside_id, 0, 43, $dId_apl_historico, 0, '$now', '', '', '')");
		$dId_historico = $db->li();
		...
		if ($dFase == 1)
		{
			//atualizar status/aba independente do status_fixo
			$db->query("UPDATE gelic_licitacoes_abas SET id_aba = 14, id_status = 19 WHERE id_licitacao = $pId_licitacao AND grupo IN (1,2,3)");
			$db->query("UPDATE gelic_licitacoes_abas SET id_aba = 8, id_status = 28 WHERE id_licitacao = $pId_licitacao AND grupo = 4");

			//remover da aba Em Participacao
...
WHERE
	apl.id = $dId_apl");
		...
		$tEmail_assunto = 'GELIC - APL Reprovada (LIC '.$db->f("id_licitacao").')';
		$tEmail_assunto_admin = $db->f("regiao_abv").'('.$db->f("uf").') - DN '.$db->f("dn").' - APL Reprovada - LIC '.$db->f("id_licitacao");

		$tEmail_mensagem = 'APL Reprovada.<br><br>
...
'.rodapeEmail();
		$tTexto_sms = 'GELIC - APL Reprovada LIC '.$db->f("id_licitacao").' LOTE: '.utf8_encode($db->f("lote")).' ITEM: '.utf8_encode($db->f("item")).' DN: '.$db->f("dn");
		...
		while ($db->nextRecord())
		{
			if (in_array("F", str_split($db->f("nt_email"))))
				queueMessage(6, 0, basename(__FILE__).' ('.__LINE__.')', $sInside_id, M_EMAIL, BOF_ADM, $db->f("id"), $db->f("email"), $tEmail_assunto_admin, $tEmail_mensagem, '', '');

			if (in_array("F", str_split($db->f("nt_celular"))) && strlen($db->f("celular")) > 0)
				queueMessage(6, 0, basename(__FILE__).' ('.__LINE__.')', $sInside_id, M_SMS, BOF_ADM, $db->f("id"), $db->f("celular"), '', $tTexto_sms, '', '');
		}
...
WHERE
	...
	cli.deletado = 0");
		while ($db->nextRecord())
		{
			...
			if (in_array("J", str_split($dNt_email["ntf"])))
				queueMessage(36, 0, basename(__FILE__).' ('.__LINE__.')', $sInside_id, M_EMAIL, BOF_DLR, $db->f("id"), $db->f("email"), $tEmail_assunto, $tEmail_mensagem, '', '');

			if (in_array("J", str_split($dNt_sms["ntf"])) && strlen($db->f("celular")) > 0)
				queueMessage(36, 0, basename(__FILE__).' ('.__LINE__.')', $sInside_id, M_SMS, BOF_DLR, $db->f("id"), $db->f("celular"), '', $tTexto_sms, '', '');
		}
		//-----------------------------------------
		//-----------------------------------------
		//-----------------------------------------
