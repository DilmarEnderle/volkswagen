...
if (isInside())
{
	...
	if ($pFormato == "xlsx")
	{
		...
		$phpexcel->getProperties()->setCreator("GELIC")
			...
			->setTitle("Novos acessos")
			->setSubject("GELIC")
			->setDescription("Novos acessos")
...
ORDER BY
	cli.id, log.id");
		...
		$phpexcel->setActiveSheetIndex(0)
			->setCellValue("A2", 'Total de acessos no período: '.$db->nf());
		...
		$phpexcel->getActiveSheet()->setTitle('Novos acessos');
		...
		if (file_exists(UPLOAD_DIR."~bo_rel_1_3_".$sInside_id.".xlsx"))
			unlink(UPLOAD_DIR."~bo_rel_1_3_".$sInside_id.".xlsx");

		$obwriter = PHPExcel_IOFactory::createWriter($phpexcel, 'Excel2007');
		$obwriter->save(UPLOAD_DIR."~bo_rel_1_3_".$sInside_id.".xlsx");
