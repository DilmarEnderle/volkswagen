...
if (isInside())
{
	...
	$pPeriodo_fr = $pPeriodo_fr_yr."-01-01";
	
	//AJUSTAR DATA PERIODO ATE PARA O FINAL DO MES
	$pPeriodo_to = $pPeriodo_to_yr."-12-31";


	$aYears = array();

	$current_yr = intval($pPeriodo_fr_yr);
	$stop_yr = intval($pPeriodo_to_yr);
	while ($current_yr <= $stop_yr)
	{
		$db->query("
SELECT
	cli.dn,
...
WHERE
	...
	YEAR(log.data_hora) = $current_yr AND
    cli.adve > 0 AND
    cli.tipo = 2 AND
	(
		cli.id IN (
...
GROUP BY
	cli.id");
		...
		$aYears[] = array("ano"=>$current_yr, "quantidade"=>$db->nf(), "dns"=>$aDns);
		$current_yr += 1;
	}
	...
	if ($pFormato == "xlsx")
	{
		...
		$phpexcel->getProperties()->setCreator("GELIC")
			...
			->setTitle("Quantidade de acessos por ano")
			->setSubject("GELIC")
			->setDescription("Quantidade de acessos por ano")
		...
		for ($i=0; $i<count($aYears); $i++)
		{
			$row += 1;
			$phpexcel->setActiveSheetIndex(0)
				->setCellValue("A$row", $aYears[$i]["ano"])
				->setCellValue("B$row", $aYears[$i]["quantidade"]);
			...
			if ($pDetalhamento == 1)
			{
				...
				if (count($aYears[$i]["dns"]) > 0)
				{
					...
					for ($j=0; $j<count($aYears[$i]["dns"]); $j++)
					{
						$row += 1;
						$phpexcel->setActiveSheetIndex(0)
							->setCellValue("A$row", $aYears[$i]["dns"][$j]["dn"])
							->setCellValue("B$row", $aYears[$i]["dns"][$j]["nome"])
							->setCellValue("C$row", $aYears[$i]["dns"][$j]["regiao"]);
					}
		...
		if (file_exists(UPLOAD_DIR."~bo_rel_2_2_".$sInside_id.".xlsx"))
			unlink(UPLOAD_DIR."~bo_rel_2_2_".$sInside_id.".xlsx");

		$obwriter = PHPExcel_IOFactory::createWriter($phpexcel, 'Excel2007');
		$obwriter->save(UPLOAD_DIR."~bo_rel_2_2_".$sInside_id.".xlsx");
