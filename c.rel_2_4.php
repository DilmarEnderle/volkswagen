...
WHERE
	...
	$aRegioes = array();
	$aRegioes[] = array("regiao"=>"Região 1/2", "acessos"=>0, "dns"=>array());
	$aRegioes[] = array("regiao"=>"Região 3", "acessos"=>0, "dns"=>array());
	$aRegioes[] = array("regiao"=>"Região 4", "acessos"=>0, "dns"=>array());
	$aRegioes[] = array("regiao"=>"Região 5", "acessos"=>0, "dns"=>array());
	$aRegioes[] = array("regiao"=>"Região 6", "acessos"=>0, "dns"=>array());

	for ($i=0; $i<count($aDns); $i++)
	{
		if ($aDns[$i]["regiao"] == 'Região 1/2')
		{
			$aRegioes[0]["acessos"] += $aDns[$i]["acessos"];
			$aRegioes[0]["dns"][] = array("dn"=>$aDns[$i]["dn"], "nome"=>$aDns[$i]["nome"], "acessos"=>$aDns[$i]["acessos"]);
		}
		else if ($aDns[$i]["regiao"] == 'Região 3')
		{
			$aRegioes[1]["acessos"] += $aDns[$i]["acessos"];
			$aRegioes[1]["dns"][] = array("dn"=>$aDns[$i]["dn"], "nome"=>$aDns[$i]["nome"], "acessos"=>$aDns[$i]["acessos"]);
		}
		else if ($aDns[$i]["regiao"] == 'Região 4')
		{
			$aRegioes[2]["acessos"] += $aDns[$i]["acessos"];
			$aRegioes[2]["dns"][] = array("dn"=>$aDns[$i]["dn"], "nome"=>$aDns[$i]["nome"], "acessos"=>$aDns[$i]["acessos"]);
		}
		else if ($aDns[$i]["regiao"] == 'Região 5')
		{
			$aRegioes[3]["acessos"] += $aDns[$i]["acessos"];
			$aRegioes[3]["dns"][] = array("dn"=>$aDns[$i]["dn"], "nome"=>$aDns[$i]["nome"], "acessos"=>$aDns[$i]["acessos"]);
		}
		else if ($aDns[$i]["regiao"] == 'Região 6')
		{
			$aRegioes[4]["acessos"] += $aDns[$i]["acessos"];
			$aRegioes[4]["dns"][] = array("dn"=>$aDns[$i]["dn"], "nome"=>$aDns[$i]["nome"], "acessos"=>$aDns[$i]["acessos"]);
		}
	}

	foreach ($aRegioes as $k => $v) $acessos_regiao[$k] = $v["acessos"];
	array_multisort($acessos_regiao, SORT_DESC, $aRegioes, SORT_NUMERIC);

	foreach ($aRegioes[0]["dns"] as $k => $v) $acessos_regiao_0[$k] = $v["acessos"];
	array_multisort($acessos_regiao_0, SORT_DESC, $aRegioes[0]["dns"], SORT_NUMERIC);

	foreach ($aRegioes[1]["dns"] as $k => $v) $acessos_regiao_1[$k] = $v["acessos"];
	array_multisort($acessos_regiao_1, SORT_DESC, $aRegioes[1]["dns"], SORT_NUMERIC);

	foreach ($aRegioes[2]["dns"] as $k => $v) $acessos_regiao_2[$k] = $v["acessos"];
	array_multisort($acessos_regiao_2, SORT_DESC, $aRegioes[2]["dns"], SORT_NUMERIC);

	foreach ($aRegioes[3]["dns"] as $k => $v) $acessos_regiao_3[$k] = $v["acessos"];
	array_multisort($acessos_regiao_3, SORT_DESC, $aRegioes[3]["dns"], SORT_NUMERIC);

	foreach ($aRegioes[4]["dns"] as $k => $v) $acessos_regiao_4[$k] = $v["acessos"];
	array_multisort($acessos_regiao_4, SORT_DESC, $aRegioes[4]["dns"], SORT_NUMERIC);
	...
	if ($pFormato == "xlsx")
	{
		...
		$phpexcel->getProperties()->setCreator("GELIC")
			...
			->setSubject("GELIC")
			->setDescription("Volume de acessos por Região")
		...
		for ($i=0; $i<count($aRegioes); $i++)
		{
			$row += 1;
			$phpexcel->setActiveSheetIndex(0)
				->setCellValue("A$row", $aRegioes[$i]["regiao"])
				->setCellValue("B$row", $aRegioes[$i]["acessos"]);
			...
			if ($pDetalhamento == 1)
			{
				if (count($aRegioes[$i]["dns"]) > 0)
				{
					$row += 1;
					$phpexcel->setActiveSheetIndex(0)
						->setCellValue("A$row", 'DN')
						->setCellValue("B$row", 'Nome do DN')
						->setCellValue("C$row", 'Acessos');
					...
					for ($j=0; $j<count($aRegioes[$i]["dns"]); $j++)
					{
						$row += 1;
						$phpexcel->setActiveSheetIndex(0)
							->setCellValue("A$row", $aRegioes[$i]["dns"][$j]["dn"])
							->setCellValue("B$row", $aRegioes[$i]["dns"][$j]["nome"])
							->setCellValue("C$row", $aRegioes[$i]["dns"][$j]["acessos"]);
					}
		...
		else
		{
			...
			$phpexcel->getActiveSheet()->getStyle("A3:B$row")->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_LEFT);
		}

		if (file_exists(UPLOAD_DIR."~bo_rel_2_4_".$sInside_id.".xlsx"))
			unlink(UPLOAD_DIR."~bo_rel_2_4_".$sInside_id.".xlsx");

		$obwriter = PHPExcel_IOFactory::createWriter($phpexcel, 'Excel2007');
		$obwriter->save(UPLOAD_DIR."~bo_rel_2_4_".$sInside_id.".xlsx");
