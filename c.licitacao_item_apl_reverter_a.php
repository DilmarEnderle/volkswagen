...
ORDER BY
	...
	$db->query("INSERT INTO gelic_licitacoes_apl_historico VALUES (NULL, $pId_apl, $sInside_id, 5, '$ip', 0, 0, '$now', '$pObservacoes')");
	$dId_apl_historico = $db->li();

	//adicionar historico tipo 44 (APL Aprovação Revertida)
	$db->query("INSERT INTO gelic_historico VALUES (NULL, $dId_licitacao, 0, $pId_apl, $sInside_id, 0, 44, $dId_apl_historico, 0, '$now', '', '', '')");
	$dId_historico = $db->li();

	//desativar registro de aprovacao
	$db->query("UPDATE gelic_licitacoes_apl_aprovadas SET ativo = 0 WHERE id_apl = $pId_apl");
	...
	//pegar aba
	$db->query("SELECT id_aba FROM gelic_licitacoes_abas WHERE id_licitacao = $dId_licitacao AND grupo = 2 ORDER BY id_aba DESC LIMIT 1");
	$db->nextRecord();
	$dId_aba = $db->f("id_aba");
...
WHERE
	...
	$tEmail_assunto = 'GELIC - APL Aprovação Revertida (LIC '.$db->f("id_licitacao").')';
	$tEmail_assunto_admin = $db->f("regiao_abv").'('.$db->f("uf").') - DN '.$db->f("dn").' - APL Aprovação Revertida - LIC '.$db->f("id_licitacao");

	$tEmail_mensagem = 'APL Aprovação Revertida.<br><br>
...
'.rodapeEmail();
	$tTexto_sms = 'GELIC - APL Aprovacao Revertida LIC '.$db->f("id_licitacao").' LOTE: '.utf8_encode($db->f("lote")).' ITEM: '.utf8_encode($db->f("item")).' DN: '.$db->f("dn");
	...
	while ($db->nextRecord())
	{
		if (in_array("G", str_split($db->f("nt_email"))))
			queueMessage(7, 0, basename(__FILE__).' ('.__LINE__.')', $sInside_id, M_EMAIL, BOF_ADM, $db->f("id"), $db->f("email"), $tEmail_assunto_admin, $tEmail_mensagem, '', '');

		if (in_array("G", str_split($db->f("nt_celular"))) && strlen($db->f("celular")) > 0)
			queueMessage(7, 0, basename(__FILE__).' ('.__LINE__.')', $sInside_id, M_SMS, BOF_ADM, $db->f("id"), $db->f("celular"), '', $tTexto_sms, '', '');
	}
...
WHERE
	...
	while ($db->nextRecord())
	{
		...
		if (in_array("K", str_split($dNt_email["ntf"])))
			queueMessage(37, 0, basename(__FILE__).' ('.__LINE__.')', $sInside_id, M_EMAIL, BOF_DLR, $db->f("id"), $db->f("email"), $tEmail_assunto, $tEmail_mensagem, '', '');

		if (in_array("K", str_split($dNt_sms["ntf"])) && strlen($db->f("celular")) > 0)
			queueMessage(37, 0, basename(__FILE__).' ('.__LINE__.')', $sInside_id, M_SMS, BOF_DLR, $db->f("id"), $db->f("celular"), '', $tTexto_sms, '', '');
	}
	//-----------------------------------------
	//-----------------------------------------
	//-----------------------------------------
