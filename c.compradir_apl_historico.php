...
FROM
	...
	INNER JOIN gelic_comprasrp AS comp ON comp.id = his.id_comprasrp
	INNER JOIN gelic_comprasrp_apl AS apl ON apl.id = his.id_apl
	INNER JOIN gelic_comprasrp_apl_historico AS ahis ON ahis.id = his.id_valor_1
...
ORDER BY 
	...
	while ($db->nextRecord())
	{
		...
		if ($db->f("tipo") == 41) //(DN) 41: APL Enviada
		{
		...
		<div class="ultimate-row lh-27 t13"'.$bg.'>
			...
			<div class="fl"><span class="bold t-orange">'.$t.'</span> por <span class="bold italic">'.$por.'</span> de <span class="t-red">'.$db->f("ip").'</span><a class="his-btn ml-10" href="'.linkFileBucket("vw/arquivos/anexo/".$db->f("arquivo")).'" target="_blank">APL</a></div>
		</div>';
		...
		else if ($db->f("tipo") == 42) //(BO) 42: APL Aprovada
		{
			...
			$rows .= '<div class="ultimate-row lh-27 t13"'.$bg.'>
				<div class="fl w-160">'.mysqlToBr(substr($db->f("data_hora"), 0, 10)).' <span class="gray-88">'.substr($db->f("data_hora"), 11).'</span></div>
				<div class="fl"><span class="bold t-green">APL aprovada</span> por <span class="bold italic">'.$por.'</span> de <span class="t-red">'.$db->f("ip").'</span> [ <a class="his-btn-info" href="javascript:void(0);" onclick="verMais('.$db->f("id").');">Ver mais...</a> ]</div>
			</div>';
...
SELECT 
	...
	ahis.texto
FROM 
	gelic_comprasrp_apl_historico AS ahis
	LEFT JOIN gelic_comprasrp_apl_aprovadas AS apr ON apr.id_apl_historico = ahis.id
WHERE 
	ahis.id = ".$db->f("id_ahis"),1);
			if ($db->nextRecord(1))
			{
				if (strlen($db->f("ave",1)) > 0)
				{
					...
					if ($db->f("arquivo",1) <> '')
						$anexo = '<tr><td class="apr-tbl-lb">ANEXO:</td><td><a class="ablue" href="'.linkFileBucket("vw/cdapr/".$db->f("arquivo",1)).'" target="_blank">'.utf8_encode($db->f("nome_arquivo",1)).'</a></td></tr>';
					else
						$anexo = '';
...
'.$valor_transf.$anexo.'
		...
		else if ($db->f("tipo") == 43) //(BO) 43: APL Reprovada
		{
			...
			$rows .= '<div class="ultimate-row lh-27 t13"'.$bg.'>
				<div class="fl w-160">'.mysqlToBr(substr($db->f("data_hora"), 0, 10)).' <span class="gray-88">'.substr($db->f("data_hora"), 11).'</span></div>
				<div class="fl"><span class="bold t-red">APL reprovada</span> por <span class="bold italic">'.$por.'</span> de <span class="t-red">'.$db->f("ip").'</span> [ <a class="his-btn-info" href="javascript:void(0);" onclick="verMais('.$db->f("id").');">Ver mais...</a> ]</div>
			</div>';
		...
		else if ($db->f("tipo") == 44) //(BO) 44: APL Aprovação Revertida
		{
			...
			$rows .= '<div class="ultimate-row lh-27 t13"'.$bg.'>
				<div class="fl w-160">'.mysqlToBr(substr($db->f("data_hora"), 0, 10)).' <span class="gray-88">'.substr($db->f("data_hora"), 11).'</span></div>
				<div class="fl"><span class="bold t-blue">Aprovação da APL revertida</span> por <span class="bold italic">'.$por.'</span> de <span class="t-red">'.$db->f("ip").'</span>'.$mais.'</div>
			</div>';
		...
		else if ($db->f("tipo") == 45) //(BO) 45: APL Reprovação Revertida
		{
			...
			$rows .= '<div class="ultimate-row lh-27 t13"'.$bg.'>
				<div class="fl w-160">'.mysqlToBr(substr($db->f("data_hora"), 0, 10)).' <span class="gray-88">'.substr($db->f("data_hora"), 11).'</span></div>
				<div class="fl"><span class="bold t-blue">Reprovação da APL revertida</span> por <span class="bold italic">'.$por.'</span> de <span class="t-red">'.$db->f("ip").'</span>'.$mais.'</div>
			</div>';
