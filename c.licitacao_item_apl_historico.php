...
if (isInside())
{
	...
	if ($sInside_tipo == 2 || $sInside_tipo == 3 || $sInside_tipo == 4) //DN, DN FILHO, REP
	{
		...
		$add_to_where = " AND (apl.id_cliente = $cliente_parent OR apl.id_cliente IN (SELECT id FROM gelic_clientes WHERE id_parent = $cliente_parent))";
	}
	else
	{
		$add_to_where = " AND (apl.id_cliente = $pId_cliente OR apl.id_cliente IN (SELECT id FROM gelic_clientes WHERE id_parent = $pId_cliente))";
	}
...
WHERE
	...
	apl.id_item = {$pId_item}{$add_to_where}
ORDER BY 
	his.id");
	while ($db->nextRecord())
	{
		...
		<div class="ultimate-row lh-27 t13"'.$bg.'>
			...
			<div class="fl"><span class="bold t-orange">'.$t.'</span> por <span class="bold italic">'.$por.'</span> de <span class="t-red">'.$db->f("ip").'</span><a class="his-btn ml-10" href="arquivos/anexo/'.$db->f("arquivo").'" target="_blank">APL</a></div>
		</div>';
				...
				if (strlen($dTexto) > 0)
				{
					$rows .= '<div id="his-'.$db->f("id").'" class="ultimate-row lh-27 t13 pb-14 his-hidden"'.$bg.'>
						...
						<div class="fl w-600" style="line-height: normal;">
							...
							<div>'.nl2br(utf8_encode($dTexto)).'</div>
						</div>
					</div>';
				}
			}
